...
description: Every mounted Kupe App path in this group (51 paths, 52 method+path pairs) and the Kupe Hub replacement.
---

Public surface is wallet, invoices, cost-summary, daily. Narrative: [billing](/migrate/billing).

**51 unique Kupe App paths** in this table (52 method+path pairs). Inventory is the live FastAPI app, not `api_specification.yaml`.

<Note>
  Kupe App also mounts the same billing handlers at `/billing/*` (no `/api/v1` prefix). Treat those as aliases of `/api/v1/billing/*` — they are not extra products.
</Note>
