...
description: Milli /api/v1/phone rows become telephony accounts. India KYC and UCC are first-class on Kupe.
---

Milli stored numbers as `phone_numbers` rows (`/api/v1/phone`). Kupe stores **telephony accounts** (`tel_...`): either a Kupe-managed Plivo DID or BYOK (Twilio / Plivo / Exotel).
...
| Milli `/api/v1/compliance-applications` (platform review queue) | **Different product** — Kupe Plivo KYC is `POST /v1/orgs/{org_id}/plivo/compliance`. Document upload is **dashboard-only**. |
...
Milli had **no** public UCC routes in the mounted API. Kupe adds org-level Plivo UCC:
