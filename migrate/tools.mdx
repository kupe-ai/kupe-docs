...
description: Milli tool-endpoint-defs, MCP server defs, and agent webhooks become org tools plus attached agent tools.
---

Milli tools were **user-owned definitions** (`/tool-endpoint-defs`, `/tools-mcp-server-defs`) then mapped onto an agent. Agent **webhooks** were a second channel: outbound “start this call” and incoming “call ended”.
...
Milli used user-level connect URLs (`/api/v1/composio/connect`, `authorize-toolkit`, `wait-for-connection`, toolkit tool lists).
...
Nango (`/api/v1/nango/*`) is **not** on the Kupe public API. Use Composio or a custom webhook.

## MCP note

Do not point Cursor at Milli. Use [Kupe MCP](/kupe-mcp): `http://mcp.kupe.in/mcp` with `Authorization: Bearer sk-kupe-...`. Tools such as `create_webhook_tool`, `create_mcp_tool`, and `list_org_tools` wrap these routes. Details: [Migrate MCP](/migrate/mcp).
