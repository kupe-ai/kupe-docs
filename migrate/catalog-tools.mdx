---
title: Catalog: tools
description: Every mounted Milli path in this group (34 paths, 41 method+path pairs) and the Kupe replacement.
---

User tool defs and agent webhooks become org tools. Narrative: [tools](/migrate/tools).

**34 unique Milli paths** in this table (41 method+path pairs). Inventory is the live FastAPI app, not `api_specification.yaml`.
...
| `/api/v1/webhooks/agents/{agent_id}/incoming` | POST | One-contact batch / MCP `trigger_outbound_call` | CRM start-call webhook is not a Kupe route. |
| `/api/v1/webhooks/agents/{agent_id}/outbound` | POST | One-contact batch / MCP `trigger_outbound_call` | CRM start-call webhook is not a Kupe route. |
