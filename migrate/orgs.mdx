...
description: Milli user-scoped orgs and API keys become Hub orgs, projects, and sk-kupe- project keys.
---

Milli organizations were a control-plane around **users** (invites, org roles, branding, bulk feature flags, custom domains). API keys belonged to a **user**.
...
| Milli | Kupe |
| --- | --- |
| `GET /api/v1/me` | `GET /v1/me` — different JSON (auth method + ids, not Milli profile/consent) |
...
Store the new secret as `KUPE_API_KEY`. Never send Milli `x-api-key` to `x.kupe.in`.
