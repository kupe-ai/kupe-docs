...
description: Every mounted Milli path in this group (40 paths, 52 method+path pairs) and the Kupe replacement.
---

Two Milli outbound APIs collapse to batches + recipient lists. Narrative: [campaigns](/migrate/campaigns).

**40 unique Milli paths** in this table (52 method+path pairs). Inventory is the live FastAPI app, not `api_specification.yaml`.
...
| `/api/v1/batch-call/{batch_id}/data` | GET | Contacts bulk / list / delete contact | `DELETE` Milli item → delete a contact on the batch if exposed; else replace via bulk. |
| `/api/v1/batch-call/{batch_id}/data/upsert` | POST | Contacts bulk / list / delete contact | `DELETE` Milli item → delete a contact on the batch if exposed; else replace via bulk. |
| `/api/v1/batch-call/{batch_id}/data/{item_id}` | DELETE | Contacts bulk / list / delete contact | `DELETE` Milli item → delete a contact on the batch if exposed; else replace via bulk. |
