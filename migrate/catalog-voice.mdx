...
description: Every mounted Milli path in this group (34 paths, 56 method+path pairs) and the Kupe replacement.
---

Milli `/api/call` webhooks and sockets, `create_call`, text/WhatsApp, Ultravox S2S. Narrative: [realtime](/migrate/realtime).

**34 unique Milli paths** in this table (56 method+path pairs). Inventory is the live FastAPI app, not `api_specification.yaml`.

<Note>
  Milli `api_specification.yaml` also lists `GET /api/call/test`. That path is **not mounted** in `milli_ai_backend` and has no Kupe replacement.
</Note>
...
| `/api/call/elison-incoming_call` | GET, POST | No equivalent | Elison is not a Kupe public product path. |
| `/api/call/elison-media-stream/agents/{agent_id}` | WS | No equivalent | Elison is not a Kupe public product path. |
| `/api/call/elison-media-stream/workflows/{workflow_id}` | WS | No equivalent | Elison is not a Kupe public product path. |
| `/api/call/elison-outbound_call` | GET, POST | No equivalent | Elison is not a Kupe public product path. |
...
| `/api/text/stream_text_ws` | WS | No equivalent | WhatsApp webhooks and text builder sockets are not public on Kupe. |
| `/api/text/stream_workflow_builder_ws` | WS | No equivalent | WhatsApp webhooks and text builder sockets are not public on Kupe. |
| `/api/text/whatsapp/webhook` | GET, POST | No equivalent | WhatsApp webhooks and text builder sockets are not public on Kupe. |
| `/api/text/whatsapp/webhook/verification-status` | GET | No equivalent | WhatsApp webhooks and text builder sockets are not public on Kupe. |
