...
description: Every mounted Kupe App path in this group (84 paths, 110 method+path pairs) and the Kupe Hub replacement.
---

Mounted Kupe App routes with no Kupe Hub public twin — workflows, WhatsApp, n8n, admin, and similar. Routes with no public twin on Kupe Hub.

**84 unique Kupe App paths** in this table (110 method+path pairs). Inventory is the live FastAPI app, not `api_specification.yaml`.
...
If you depended on any of these, rebuild the workflow with a Kupe Hub agent, org tools, Hub, or a provider you own. Do not assume a hidden alias on `x.kupe.in`.
