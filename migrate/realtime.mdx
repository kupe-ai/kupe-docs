...
description: Milli media sockets and POST /api/call/create_call become Kupe realtime sessions and one-contact batches.
---

Milli mixed three things on `/api/call`: provider **webhooks** (TwiML/XML), **WebSockets** for media, and **`POST /create_call`** for a single outbound PSTN dial. Kupe splits those.
...
Milli `POST /api/call/create_call`:
...
Kupe has **no** `create_call`. A single dial is a **one-contact campaign**:
...
MCP wraps that as `trigger_outbound_call`. There is **no** `workflow_id` — Kupe does not run Milli workflows. See [Campaigns](/guides/campaigns).
...
Milli exposed raw provider WebSockets:
...
Kupe telephony media is `wss://x.kupe.in/agents/v1/media?ticket=...&provider=...`. Tickets are issued by **inbound/outbound webhooks**, not by your client. They are not in the public OpenAPI. Point Plivo/Twilio/Exotel applications at Kupe’s webhook URLs (set on [purchase](/guides/phones) for managed numbers), not at Milli sockets.

## Provider HTTP webhooks

| Milli | Kupe (platform-owned; not public API) |
| --- | --- |
...
| `.../elison-incoming_call` / `elison-outbound_call` | No Elison product path on Kupe public API |
...
`WS /api/text/stream_text_ws`, workflow-builder WS, and `/api/text/whatsapp/webhook` have **no Kupe public equivalent**. WhatsApp is not in the public OpenAPI.
