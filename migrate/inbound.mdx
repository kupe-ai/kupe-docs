...
description: Milli phone-number mappings become inbound deployments with availability windows.
---

Milli inbound was a **mapping row**: agent (or workflow) + `phone_number_id` + optional webhook URL.
...
Milli `POST /api/v1/phone/mapping` body (`AgentPhoneMappingCreate`): `agent_id` **xor** `workflow_id`, `phone_number_id`, optional `webhook_url`, `config_source`, `user_id`.

Kupe `POST /v1/inbound` requires `org_id`, `project_id`, `agent_id`, `telephony_account_id`, `name`. Optional `status` (`draft` | `active` | `paused`) and `availability`.

There is **no** `workflow_id`. Milli `POST /api/v1/phone/mapping/workflow` has no replacement.
...
`availability.always: true` (default) ignores days and hours. `days_of_week` is `0=Sun … 6=Sat`. Milli mappings had **no** hours window — that is new.
