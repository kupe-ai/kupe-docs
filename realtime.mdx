...
or stop sending `input_audio_buffer.append` while the agent's audio is still
playing. Track that from the bytes you have queued for playback, not from the
arrival of the last delta — deltas arrive faster than realtime. Suppressing
the mic this way means the caller cannot barge in.
