...
`GET /v1/me` fills in `org_id` / `project_id` for API keys, so the SDK constructor can be just `Kupe()`. Pass `voice` (name) or `voice_id` — either one.
...
The WebSocket speaks OpenAI-compatible JSON. Audio is **PCM16 mono at 24 kHz**. See [Realtime](/realtime) for client and server events. SDK-only walkthroughs: [Python](/sdk/python/realtime) and [TypeScript](/sdk/typescript/realtime).
