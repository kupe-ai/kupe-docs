...
<div class="kupe-hero">
  ...
  <div class="kupe-hero-copy">
    <div class="kupe-kicker">Coding agents</div>
    <h1>Kupe MCP</h1>
    <p>
      One API key. One remote URL. Your coding agent can create agents, launch
      campaigns, buy numbers, read transcripts, and check wallet balance —
...
<Warning>
  **`https://docs.kupe.in/mcp` is not this page.** Mintlify reserves `/mcp` for
  its own docs-search MCP (raw JSON in the browser). The **Kupe platform** MCP
  lives at **`http://mcp.kupe.in/mcp`**. This guide is at
  [`/kupe-mcp`](/kupe-mcp).
</Warning>

<CardGroup cols={3}>
  <Card title="Cursor" icon="sparkles">
    Paste the remote JSON into Settings → MCP. Tools appear under `kupe`.
  </Card>
  <Card title="Claude Code" icon="terminal">
    One `claude mcp add` command with your Bearer key.
  </Card>
  <Card title="Codex" icon="bot">
...
Opening that URL in Safari or Chrome returns **Not Acceptable** — that is expected. It is an MCP wire protocol, not a website. Use an MCP client.
...
Also: knowledge bases, call databases, org tools, and Composio connections. Full map: [kupe-mcp tool reference](https://github.com/kupe-ai/kupe-mcp/blob/main/docs/TOOLS.md).
...
<AccordionGroup>
  ...
  <Accordion title="Two different MCPs">
    - **`http://mcp.kupe.in/mcp`** — Kupe platform (this page).
    - **`https://docs.kupe.in/mcp`** — Mintlify docs search only (read-only site content).
  </Accordion>
</AccordionGroup>
