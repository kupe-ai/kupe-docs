...
Voices are addressed by **sanitized name** (`priya`) **or voice id** on realtime sessions. Copy the id from the voice library. Cloned names must be unique. `GET /v1/voices` returns catalog rows, public clones, and — when you send a user JWT — your private clones.
...
- `POST /v1/voices/clone` — `multipart/form-data`: `name` (unique after sanitize), `sample` (≤ 25 MB), optional `is_public`. Duplicate names return `409`.
- `PATCH /v1/voices/{voice_id}` — rename or visibility. Rename must stay unique (`409` if taken).
