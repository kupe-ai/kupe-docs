...
Pass `echoSuppression: "half_duplex"` to drop mic chunks while the agent is
still speaking:

```ts
const rt = await kupe.realtime.connect(session, { echoSuppression: "half_duplex" });

rt.appendAudio(pcm16); // dropped while the agent talks
```
...
dropped; `rt.suppressedChunks` counts the drops, and `rt.agentIsSpeaking`
